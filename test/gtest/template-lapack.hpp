    ...
    for (int l = 0; l < Nloop; l++)
      {
        ...
        complexdouble zero(0,0),one(1,0);
        MltAdd(1., U@storage_blasGE_@real, S@storage_blasGE_@real, 0., tmp@storage_blasGE_@real);
        MltAdd(one, U@storage_blasGE_@complex, S@storage_blasGE_@complex, zero, tmp@storage_blasGE_@complex);
        S@storage_blasGE_@real_complex = tmp@storage_blasGE_@real_complex;
        MltAdd(1., S@storage_blasGE_@real, V@storage_blasGE_@real, 0., tmp@storage_blasGE_@real);
        MltAdd(one, S@storage_blasGE_@complex, V@storage_blasGE_@complex, zero, tmp@storage_blasGE_@complex);
        for (int i = 0; i < m; i++)
          for (int j = 0; j < n; j++)
            {
              ASSERT_NEAR(tmp@storage_blasGE_@real(i, j), A@storage_blasGE_@real(i, j), 1.e-2 * A@storage_blasGE_@real(i, j));
              ASSERT_NEAR(tmp@storage_blasGE_complex@real(i, j).real(), A@storage_blasGE_complex@real(i, j).real(), 1.e-2 * A@storage_blasGE_complex@real(i, j).real());
              ASSERT_NEAR(tmp@storage_blasGE_complex@real(i, j).imag(), A@storage_blasGE_complex@real(i, j).imag(), 1.e-2 * A@storage_blasGE_complex@real(i, j).imag());
            }
      }
  }
