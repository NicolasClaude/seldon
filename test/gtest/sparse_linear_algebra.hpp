...
class SparseLinearAlgebraTest: public testing::Test
{
protected:
  int m_;
  int n_;
...
/*
        ...
        for (int i = 0; i < m_; i++)
          for (int j = 0; j < p_; j++)
            ASSERT_NEAR(C_full(i, j), C_full_2(i, j),
                                         1.e-14 * C_full(i, j));
        Transpose(B);
        Transpose(B_full);
        C_full.Fill(0);
...
TEST_F(SparseLinearAlgebraTest, TestTrans)
  {
    Nloop_ = 1;
...
TEST_F(SparseLinearAlgebraTest, TestAdd)
  {
    Nloop_ = 1;
...
TEST_F(SparseLinearAlgebraTest, TestMlt)
  {
    Nloop_ = 1;
